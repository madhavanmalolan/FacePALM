...
class tweets:
	...
	# "unclustered_local" directory should exist in the directory from where the script is run.
...
class clusterHTTPHandler(BaseHTTPServer.BaseHTTPRequestHandler):
	...
	self.wfile.write(response)
	    ...
	    ###os.system(mahout_kmeans_command)
            ...
            get_list_centroids_command = "hadoop jar centroids.jar GetClusters /twitter_%d/kmeans/clusters-*-final /twitter_%d/centroids_list -libjars ~/Softwares/Apache/mahout-distribution-0.7/mahout-math-0.7.jar"%(ITER,ITER)
	    os.system(get_list_centroids_command)
	    ...
	    hdfs_file_links['centroids'] =  HDFS_BASE_DIR+"centroids_list/part-m-00000"
	    hdfs_file_links['centroid_file'] = "/twitter_%d/canopy/part-randomSeed"%ITER
	    json_hdfs_files = json.dumps(hdfs_file_links)
            print json_hdfs_files

            params = unicode(json_hdfs_files)
